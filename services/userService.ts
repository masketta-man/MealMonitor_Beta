...
export const userService = {
  ...
  async getProfile(userId: string): Promise<UserProfile | null> {
    // maybeSingle rather than single: a user who hasn't finished onboarding has no
    // row yet, and that's an expected state, not an error worth logging.
  ...
  async getUserStats(userId: string) {
    const [profile, completedMeals, challengeProgress, badges] =
      await Promise.all([
