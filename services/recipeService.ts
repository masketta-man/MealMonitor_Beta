...
export const recipeService = {
  ...
  async getRecipe(
    ...
    if (userId) {
      const [enrichedRecipes] = await this.enrichRecipesWithUserData(
        [recipe],
        userId,
      )
  ...
  async getRecommendations(
    ...
    const [userIngredients, userProfile, userSettings, todaysLog] =
      await Promise.all([
        supabase
    ...
    const availableIngredientNames = (userIngredients.data || []).map(
      (ui: any) => ui.ingredients.name,
    )
    ...
    console.log("🍽️ Meal Recommendations:", {
      ...
      remainingCalories,
      dietaryPreferences,
      topScores: filteredRecipes.slice(0, 3).map((r) => ({
        title: r.title,
        score: r.score,
  ...
  async completeRecipe(
    ...
    console.log(" recipeService.completeRecipe: Starting...", {
      userId,
      recipeId,
      awardPoints,
      servingsScale,
    ...
    if (checkError) {
      console.error(
        " recipeService.completeRecipe: Error checking completions:",
        checkError,
      )
    }
    ...
    if (recipeError || !recipe) {
      console.error(
        " recipeService.completeRecipe: Error fetching recipe:",
        recipeError,
      )
    ...
    console.log("📊 recipeService.completeRecipe: Recipe details:", {
      title: recipeTitle,
      points: recipePoints,
    ...
    // Add to completed meals and get the inserted record ID
    ...
    if (insertError || !completion) {
      console.error(
        " recipeService.completeRecipe: Error inserting completion:",
        insertError,
      )
      return { success: false, leveledUp: false }
    }
    ...
    try {
      ...
      console.error(
        "Error advancing quests after recipe completion:",
        questError,
      )
    }
    ...
    if (shouldAwardPoints && pointsToAward > 0) {
      ...
      if (newBadges.length > 0) {
        console.log(
          `🏆 User ${userId} earned ${newBadges.length} new badge(s) after completing recipe!`,
        )
      }

      return { success: true, leveledUp, newLevel, oldLevel, completionId }
    } else {
      console.log(
        " recipeService.completeRecipe: No points awarded (already completed today or points disabled)",
      )
    }

    return { success: true, leveledUp: false, completionId }
  },
  ...
  async enrichRecipesWithUserData(
    recipes: RecipeWithDetails[],
    userId: string,
  ): Promise<RecipeWithDetails[]> {
    // Get user data in parallel
    const [favoritesData, userIngredientsData, userSettings] =
      await Promise.all([
        supabase
          .from("user_favorites")
          .select("recipe_id")
    ...
    const favoriteIds = new Set(
      favoritesData.data?.map((f: any) => f.recipe_id) || [],
    )
    ...
    const availableIngredientNames = new Set(
      userIngredientsData.data?.map((ui: any) =>
        normalizeIngredientName(ui.ingredients.name),
      ) || [],
    )
    ...
    return filteredRecipes.map((recipe) => ({
      ...
      hasAllIngredients: !hasIngredientsTracked
        ? true
  ...
  async getEnhancedRecommendations(
    ...
    const calorieGoal =
      todaysLog?.calorie_goal || settings?.daily_calorie_target || 2000
    const currentCalories = todaysLog?.total_calories || 0
    const remainingCalories = calorieGoal - currentCalories
